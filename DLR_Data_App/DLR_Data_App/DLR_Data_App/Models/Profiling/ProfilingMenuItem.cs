    ...
    public class ProfilingMenuItem : BindableObject
    {
        ...
        public ProfilingMenuItem(string id, string chapterName, int answersNeeded, List<int> introspectionQuestions)
        {
            ...
            if (ProfilingPageType == null || ProfilingPageType.IsAssignableFrom(typeof(IProfilingPage)))
                throw new ArgumentException($"You need to provide an IProfilingPage matching the id given (for given id it needs to be called {id.ToString()}Page and must be located in {nspace}.Views.Profiling");
            ...
            if (QuestionType == null || QuestionType.IsAssignableFrom(typeof(IQuestionContent)))
                throw new ArgumentException($"You need to provide a IQuestionContent matching the id given (for given id it needs to be called Question{id.ToString()}Page and must be located in {nspace}.Models.Profiling");
            ...
            if (AnswerType == null || AnswerType.IsAssignableFrom(typeof(IUserAnswer)))
                throw new ArgumentException($"You need to provide a ProfilingPage matching the id given (for given id it needs to be called Answer{id.ToString()}Page and must be located in {nspace}.Models.Profiling");
